...
def _autosave_to_disk(explicit_folder: str = ""):
    ...
    # Normalise: strip trailing slashes, convert forward-slashes to os.sep
    folder = folder.rstrip("/\").replace("/", os.sep).replace("\\", os.sep)
...
with st.sidebar:
    ...
    _can_save = (
        bool(st.session_state.get("autosave_path", "").strip())
        and st.session_state.df_original is not None
    )
    if st.button("💾 Create / Save Excel Now", use_container_width=True, disabled=not _can_save):
        # Read path directly from the widget key — guaranteed to be current
        # even if session_state["autosave_path"] lags behind the widget value.
        _folder = st.session_state.get("autosave_path_input",
                    st.session_state.get("autosave_path", "")).strip()
        _folder = _folder.rstrip("/\").replace("/", os.sep)
        if _folder.lower().endswith(".xlsx"):
            _folder = os.path.dirname(_folder)
        # Keep session state in sync with whatever was actually used
        st.session_state["autosave_path"] = _folder
        _autosave_to_disk(explicit_folder=_folder)
        _today_fname = "Reconciliation_" + _ist_now().strftime("%Y-%m-%d") + ".xlsx"
        _full = os.path.join(_folder, _today_fname)
        if os.path.exists(_full):
            st.toast("✅ Saved: " + _full, icon="💾")
        st.rerun()
    st.divider()
