...
def _autosave_to_disk():
    ...
    if path:
        ...
        try:
            ...
            st.toast("⚠️ Local save skipped — file open in Excel.", icon="🔒")
        except Exception as e:
            st.toast(f"⚠️ Local save failed: {e}", icon="❌")
...
with st.sidebar:
    st.markdown(
        ...
        "<span style='font-size:1.05rem;font-weight:700;color:#1e40af'>◈ AccountSync</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    autosave_path = st.text_input(
        ...
        placeholder=f"e.g. C:/exports/{_default_fname}",
        label_visibility="collapsed",
        key="autosave_path_input",
        on_change=lambda: st.session_state.update({"autosave_path": st.session_state.get("autosave_path_input", "")}),
    )
    if autosave_path:
        st.session_state["autosave_path"] = autosave_path
        st.caption(f"📂 {autosave_path}")
    # Button to create / overwrite the Excel file at the given path right now
    _can_save = bool(st.session_state.get("autosave_path", "").strip()) and st.session_state.df_original is not None
    if st.button("💾 Create / Save Excel Now", use_container_width=True, disabled=not _can_save):
        _save_path = st.session_state.get("autosave_path", "").strip()
        # Auto-append default filename if user gave only a directory
        if not _save_path.endswith(".xlsx"):
            import os as _os
            _save_path = _os.path.join(_save_path, _default_fname)
            st.session_state["autosave_path"] = _save_path
        _autosave_to_disk()
        st.toast(f"✅ Saved to {_save_path}", icon="💾")
    ...
    if st.button("🗑 Delete Row", use_container_width=True, disabled=not _data_loaded):
        if _data_loaded:
            st.toast("Select row(s) in the table, then use the Delete button that appears.", icon="ℹ️")
