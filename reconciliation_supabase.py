...
with st.sidebar:
    ...
    autosave_path = st.text_input(
        ...
        placeholder=r"e.g. C:\Users\ASUS\Downloads\accounts",
        label_visibility="collapsed",
        key="autosave_path_input",
    )

    if autosave_path:
        # Clean the typed path — strip quotes, normalize slashes
        _folder = autosave_path.strip().replace('"', '').replace("'", "")
        _folder = _os.path.normpath(_folder)
        # If user typed a full .xlsx path, extract just the folder
        if _folder.lower().endswith(".xlsx"):
            _folder = _os.path.dirname(_folder)
        # Always build the full file path as folder + dated filename
        _full_path = _os.path.join(_folder, _default_fname)
        st.session_state["_autosave_folder"] = _folder
        st.session_state["autosave_path"]    = _full_path

    _stored_path = st.session_state.get("autosave_path", "").strip()

    if _stored_path:
        st.caption(f"\U0001f4c4 Will save to: **{_stored_path}**")

    # Last save timestamp (IST)
    _last_save_ts = st.session_state.get("_last_autosave", "")
    if _last_save_ts:
        st.success(f"\u2705 Last saved: {_last_save_ts}")

    # Button — create / overwrite file immediately
    _can_save = bool(_stored_path) and st.session_state.df_original is not None
    if st.button("\U0001f4be Create & Save Now", use_container_width=True,
                 disabled=not _can_save, key="btn_create_save"):
        _autosave_to_disk()
        _ts = st.session_state.get("_last_autosave", "")
        if _ts:
            st.toast(f"\u2705 Saved: {_stored_path}", icon="\U0001f4be")
        st.rerun()
    st.divider()
