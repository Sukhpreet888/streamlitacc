...
# ══════════════════════════════════════════════════════════
#  AUTH GATE — handle query-param login submission first,
#  then show login page if not signed in
# ══════════════════════════════════════════════════════════
# Handle HTML-card login form submission (sets ?lgu=...&lgp=...)
_qp = st.query_params
if "lgu" in _qp and "lgp" in _qp:
    _u = _qp.get("lgu", "").strip()
    _p = _qp.get("lgp", "").strip()
    st.query_params.clear()
    if _u and _p:
        from auth import verify_login, do_login
        _ok, _result = verify_login(_u, _p)
        if _ok:
            do_login(_u, _result)
            st.rerun()
        else:
            st.session_state["_login_error"] = _result
            st.rerun()
    else:
...
def _autosave_to_disk():
    """Save df_original to local disk AND Supabase cloud on every edit."""
    ...
    # ── A. Local disk save (atomic temp-file replace) ─────
    path = st.session_state.get("autosave_path", "").strip()
    if path:
        if not path.lower().endswith(".xlsx"):
            path = os.path.join(path, f"reconciliation_{datetime.now().strftime('%Y-%m-%d')}.xlsx")
        ...
        try:
            ...
            tmp_fd, tmp_path = tempfile.mkstemp(dir=dir_ or ".", suffix=".xlsx")
            try:
                with os.fdopen(tmp_fd, "wb") as tmp_f:
                    tmp_f.write(wb_bytes)
            ...
            st.toast("⚠️ File is open in Excel — close it then save again.", icon="🔒")
        except FileNotFoundError as e:
            st.toast(f"⚠️ Folder not found: {e} — check your path.", icon="❌")
        except Exception as e:
            # Show full error so user knows exactly what went wrong
            st.toast(f"⚠️ Save failed: {type(e).__name__}: {e}", icon="❌")
...
with st.sidebar:
    ...
    _default_fname = f"reconciliation_{datetime.now().strftime('%Y-%m-%d')}.xlsx"

    # Read widget value directly — don't rely on on_change which fires one
    # rerun late. Grab whatever is in the input RIGHT NOW and sync immediately.
    autosave_path = st.text_input(
        "Save path",
        value=st.session_state.get("autosave_path", ""),
        placeholder=f"e.g. D:/exports/{_default_fname}",
        label_visibility="collapsed",
        key="autosave_path_input",
    )
    # Sync to session_state on every render, not just on_change
    if autosave_path:
        st.session_state["autosave_path"] = autosave_path.strip()

    # Show current stored path
    _stored_path = st.session_state.get("autosave_path", "").strip()
    if _stored_path:
        st.caption(f"📂 {_stored_path}")

    # Show last save timestamp if available
    _last_save_ts = st.session_state.get("_last_autosave", "")
    if _last_save_ts:
        st.success(f"✅ Last saved: {_last_save_ts}", icon="💾")

    # Create / Save button
    _can_save = bool(_stored_path) and st.session_state.df_original is not None
    if st.button("💾 Create / Save Excel Now", use_container_width=True, disabled=not _can_save):
        import os as _os
        _save_path = _stored_path
        # Auto-append filename if only a directory was given
        if not _save_path.lower().endswith(".xlsx"):
            _save_path = _os.path.join(_save_path, _default_fname)
            st.session_state["autosave_path"] = _save_path
        # Force the path into autosave_to_disk before calling it
        st.session_state["autosave_path"] = _save_path
        _autosave_to_disk()
        if st.session_state.get("_last_autosave"):
            st.toast(f"✅ Saved → {_save_path}", icon="💾")
        st.rerun()
...
# ══════════════════════════════════════════════════════════
    ...
    if confirm:
        ...
        try:
            ...
            st.session_state["_last_sb_save"] = datetime.now().strftime("%H:%M:%S")
        except Exception as e:
