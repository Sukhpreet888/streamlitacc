...
def _autosave_to_disk():
    ...
    if df is None:
        return
    now_ist = _ist_now()
    ts      = now_ist.strftime("%I:%M:%S %p IST")   # e.g. 07:39:13 PM IST
    today   = now_ist.strftime("%Y-%m-%d")

    # ── A. Local disk save (atomic temp-file replace) ─────
    path = st.session_state.get("autosave_path", "").strip()
    if path:
        # Auto-append dated filename if only a folder was given
        if not path.lower().endswith(".xlsx"):
            path = os.path.join(path, f"reconciliation_{today}.xlsx")
...
with st.sidebar:
    ...
    _today_ist   = _ist_now().strftime("%Y-%m-%d")
    _default_fname = f"reconciliation_{_today_ist}.xlsx"
    ...
    # Sync typed value into session_state immediately on every render
    _prev_path = st.session_state.get("autosave_path", "").strip()
    _typed_path = autosave_path.strip() if autosave_path else ""

    if _typed_path:
        import os as _os
        # Normalise: convert forward slashes to OS separator, strip quotes
        _typed_path = _typed_path.replace('"', '').replace("'", "").strip()
        _typed_path = _os.path.normpath(_typed_path)

        # Auto-append filename if user gave only a folder (no .xlsx at end)
        if not _typed_path.lower().endswith(".xlsx"):
            _typed_path = _os.path.join(_typed_path, _default_fname)

        # If path changed (newly typed or corrected), create the file immediately
        if _typed_path != _prev_path:
            st.session_state["autosave_path"] = _typed_path
            if st.session_state.df_original is not None:
                _autosave_to_disk()
                if st.session_state.get("_last_autosave"):
                    st.toast(f"✅ File created at: {_typed_path}", icon="💾")
                # else autosave already showed error toast
            else:
                st.toast("📝 Path saved — file will be created when you load data.", icon="ℹ️")

    _stored_path = st.session_state.get("autosave_path", "").strip()
    if _stored_path:
        st.caption(f"📂 {_stored_path}")

    # Last save timestamp (IST)
    _last_save_ts = st.session_state.get("_last_autosave", "")
    if _last_save_ts:
        st.success(f"✅ Last saved: {_last_save_ts}", icon="💾")

    # Manual save button (also useful if file was deleted externally)
    _can_save = bool(_stored_path) and st.session_state.df_original is not None
    if st.button("💾 Save Now", use_container_width=True, disabled=not _can_save):
        st.session_state["autosave_path"] = _stored_path
        _autosave_to_disk()
        st.toast(f"✅ Saved → {_stored_path}", icon="💾")
        st.rerun()
