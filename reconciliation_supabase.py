...
with st.sidebar:
    ...
    _default_fname = "reconciliation_" + _today_ist + ".xlsx"

    _folder_input = st.text_input(
        "Folder path",
        value=st.session_state.get("_autosave_folder", ""),
    ...
    if _folder_input:
        # Strip any surrounding quotes the user may have copied
        _f = _folder_input.strip().strip('"').strip("'").strip()
        # Replace forward slashes with backslash on Windows
        _f = _f.replace("/", _os.sep)
        # Normalise the path (handles double slashes etc.)
        _f = _os.path.normpath(_f)
        # If user accidentally typed a full .xlsx path, use its parent folder
        if _f.lower().endswith(".xlsx"):
            _f = _os.path.dirname(_f)
        # Build full save path: folder + reconciliation_YYYY-MM-DD.xlsx
        _full = _os.path.join(_f, _default_fname)
        st.session_state["_autosave_folder"] = _f
        st.session_state["autosave_path"]    = _full

    _stored_path = st.session_state.get("autosave_path", "").strip()

    if _stored_path:
        st.caption("📄 Will save to:")
        st.code(_stored_path, language=None)

    _last_save_ts = st.session_state.get("_last_autosave", "")
    if _last_save_ts:
        st.success("✅ Last saved: " + _last_save_ts)
    else:
        if _stored_path:
            st.info("ℹ️ Click the button below to create the file.")

    _can_save = bool(_stored_path) and st.session_state.df_original is not None
    if st.button("💾 Create & Save Now", use_container_width=True,
                 disabled=not _can_save, key="btn_create_save"):
        try:
            _dir = _os.path.dirname(_os.path.abspath(_stored_path))
            _os.makedirs(_dir, exist_ok=True)
            _autosave_to_disk()
            _ts = st.session_state.get("_last_autosave", "")
            if _ts:
                st.toast("✅ File created: " + _stored_path, icon="💾")
            else:
                st.error("Save failed — check path and try again.")
        except Exception as _e:
            st.error(f"Error creating file: {_e}")
        st.rerun()
    st.divider()
