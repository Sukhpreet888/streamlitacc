...
def _autosave_to_disk():
    ...
    # ── A. Local disk save ────────────────────────────────
    # Always read the *folder* path from session state (never a .xlsx path).
    # Compute today's filename fresh on every call so the date is always current.
    folder = st.session_state.get("autosave_path", "").strip()
    # Strip an accidentally stored full .xlsx path back to its parent folder
    # (guards against any pre-existing corrupted session state from the old bug)
    if folder.lower().endswith(".xlsx"):
        folder = os.path.dirname(folder)
    if folder:
        # Validate that the folder exists before attempting to write
        if not os.path.isdir(folder):
            st.toast(
                f"⚠️ Auto-save path does not exist: {folder}",
                icon="❌",
            )
        else:
            # Build today's dated filename — recomputed every call
