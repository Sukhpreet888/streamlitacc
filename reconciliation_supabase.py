...
def _autosave_to_disk(explicit_folder: str = ""):
    """Save df_original to local disk + Supabase on every edit.

    explicit_folder: when called from a button handler, pass the folder path
    directly so there is no risk of reading a stale session-state value.
    """
    ...
    # ── A. Local disk save ────────────────────────────────
    # Prefer the explicitly-passed folder; fall back to session state.
    folder = (explicit_folder or st.session_state.get("autosave_path", "")).strip()
    # Normalise: let os.path.normpath handle slash variants and trailing separators
    if folder:
        folder = os.path.normpath(folder)
    # If an old full .xlsx path was stored, recover the parent folder
    if folder.lower().endswith(".xlsx"):
        folder = os.path.dirname(folder)
    if folder:
        # Create the folder if it does not exist yet (handles new sub-folders)
        try:
            os.makedirs(folder, exist_ok=True)
        except Exception as e:
            st.toast(f"⚠️ Cannot create folder: {e}", icon="❌")
            folder = ""   # skip the write
        if folder:
            # Build today's dated filename — recomputed on every call
            today_fname = "Reconciliation_" + _ist_now().strftime("%Y-%m-%d") + ".xlsx"
            save_path = os.path.join(folder, today_fname)
            try:
                df_exp = df[[c for c in df.columns if c != "_matched"]]
                wb_bytes = _export_styled_excel(df_exp)
                with open(save_path, "wb") as f:
                    f.write(wb_bytes)
                st.session_state["_last_autosave"] = ts
            except Exception as e:
                st.toast(f"⚠️ Auto-save failed: {e}", icon="❌")

    # ── B. Supabase cloud save ────────────────────────────
    ...
    if session_id:
        try:
            ...
            st.toast(f"⚠️ Supabase save failed: {e}", icon="❌")
...
with st.sidebar:
    ...
    # ── 4. Auto-Save Path ─────────────────────────────────
    st.markdown("#### 💾 Auto-Save Path")
    _default_fname = "reconciliation_" + _ist_now().strftime("%Y-%m-%d") + ".xlsx"

    autosave_path = st.text_input(
        "Save path",
        value=st.session_state.get("autosave_path", ""),
        placeholder="e.g. C:\\Users\\ASUS\\Downloads\\accounts",
        label_visibility="collapsed",
        key="autosave_path_input",
        on_change=lambda: st.session_state.update(
            {"autosave_path": st.session_state.get("autosave_path_input", "")}
        ),
    )
    if autosave_path:
        st.session_state["autosave_path"] = autosave_path
        st.caption("📂 " + autosave_path)

    # Always read the live widget value — it is current regardless of on_change timing
    _live_path = st.session_state.get("autosave_path_input", "").strip()
    # Also sync session state so _autosave_to_disk (called from other triggers) sees it
    if _live_path:
        st.session_state["autosave_path"] = _live_path

    _can_save = bool(_live_path) and st.session_state.df_original is not None

    if st.button("💾 Create / Save Excel Now", use_container_width=True, disabled=not _can_save):
        _folder = os.path.normpath(_live_path) if _live_path else ""
        if _folder.lower().endswith(".xlsx"):
            _folder = os.path.dirname(_folder)
        st.session_state["autosave_path"] = _folder
        # Attempt the save and show a persistent result — not just a toast
        try:
            os.makedirs(_folder, exist_ok=True)
            _today_fname = "Reconciliation_" + _ist_now().strftime("%Y-%m-%d") + ".xlsx"
            _full = os.path.join(_folder, _today_fname)
            _df_exp = st.session_state.df_original[
                [c for c in st.session_state.df_original.columns if c != "_matched"]
            ]
            _wb = _export_styled_excel(_df_exp)
            with open(_full, "wb") as _fh:
                _fh.write(_wb)
            st.session_state["_last_autosave"] = _ist_now().strftime("%I:%M:%S %p IST")
            st.session_state["_last_autosave_path"] = _full
            # Also sync to Supabase
            _sid = st.session_state.get("supabase_session_id")
            if _sid:
                try:
                    save_dataframe(_sid, st.session_state.df_original)
                    st.session_state["_last_sb_save"] = st.session_state["_last_autosave"]
                except Exception:
                    pass
        except Exception as _e:
            st.session_state["_last_autosave_err"] = str(_e)
        st.rerun()

    # Show save result below the button (persists across reruns unlike toasts)
    _last_path = st.session_state.get("_last_autosave_path", "")
    _last_ts   = st.session_state.get("_last_autosave", "")
    _last_err  = st.session_state.pop("_last_autosave_err", "")
    if _last_err:
        st.error(f"❌ Save failed: {_last_err}")
    elif _last_path and _last_ts:
        st.success(f"✅ {_last_ts}")
        st.caption(f"📄 {os.path.basename(_last_path)}")
    st.divider()
